...
class MotorController:
    ...
    def __init__(self,enable_motors=True, command_cooldown=0.01):
        """
        Initialize the motor controller
        
        Args:
            control_mode: "gpio" or "arduino"
            arduino_port: Serial port for Arduino (if using arduino mode)
            baud_rate: Baud rate for Arduino communication
        ...
        # Arduino setup for serial control
    ...
    def send_command(self, command, source="manual"):
        ...
        """
        # Rate limiting - don't send the same command repeatedly
        ...
        if self.control_mode == "gpio" and GPIO_AVAILABLE:
            ...
            logger.warning(f"Unable to send command - invalid control mode or not connected")
            return False
    ...
    def process_joystick_input(self, forward, turn):
        ...
        Returns:
            command: The motor command sent
        """
        # Convert joystick values to motor command
        ...
        if abs(turn) > abs(forward):
            # Turning dominates
            if turn > 0.1:
                return self.send_command(self.CMD_RIGHT, "joystick")
            elif turn < -0.1:
                return self.send_command(self.CMD_LEFT, "joystick")
        else:
            # Forward/backward dominates
            if forward > 0.1:
                return self.send_command(self.CMD_FORWARD, "joystick")
            elif forward < -0.1:
                return self.send_command(self.CMD_BACKWARD, "joystick")
